...
**The LIVE hand-card layout is `_build_chart_proto()` (v8 "campaign writ", 2026-06-11)** — the card IS the campaign's aged parchment with the document furniture printed on it: per-rarity metal double rules over an ink keyline (bronze/silver/gold/pewter; curse = bone on murk-stained paper), art mounted big as an ink-matted gilt-filleted plate (nothing is ever pinned over the painting), dark-ink name cartouche with gilt Cinzel (auto-shrinks long names), rules text written straight on the page inside a hairline ink box with metal corner ticks (keyword gold is re-inked `#7a4f10` there — the tooltip gold `#e8b547` washes out on paper), keyword devices ink-stamped in the bottom margin, and stats as pressed wax seals (`WaxSeal` class — seeded irregular blob per card+stat; paper tooth is the seeded `ParchmentPlate` painter, rounded-corner aware so nothing leaks past the leaf radius). Battlefield tokens are `_build_compact_layout` in the same material kit (38px wax seals via the shared `_chart_seal`, 18px outlined numerals). **Geometry gotcha: the Card2D control is natively 225×300 — absolute offsets are real pixels; only `_center_at_point` maps the 300×400 design space** (where v6/v3 numbers like `(150, 213)` live). Cost/ATK/HP keep the historical 56px corner boxes so `_build_baked_overlay_layout` numerals land on them. A/B harnesses live in `tools/card_redesign_v6/` (`render_v7.tscn` = current, `render_cards.tscn` = v6/v3). Everything below describes the SUPERSEDED v3 painted-frame layout, kept for reference:
